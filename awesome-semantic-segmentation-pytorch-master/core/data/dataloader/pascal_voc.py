...
class VOCSegmentation(SegmentationDataset):
    ...
    def __init__(self, root=r'G:\Machine\songxianchong\train', split='train', mode=None, transform=None, **kwargs):
        super(VOCSegmentation, self).__init__(root, split, mode, transform, **kwargs)
        _voc_root = os.path.join(root, self.BASE_DIR)
        _mask_dir = os.path.join(_voc_root, 'mask')
        ...
        for i in os.listdir(_image_dir):
            self.images.append(_image_dir + os.sep + i)
            self.masks.append(_mask_dir + os.sep + i)
